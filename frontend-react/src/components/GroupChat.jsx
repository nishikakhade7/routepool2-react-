...
export default function GroupChat({ groupId, groupName, route, onClose, inline = false }) {
  const { user } = useAuth();
  ...
  const chatContent = (
    <div className="chat-drawer" style={inline ? { width: '100%', borderLeft: 'none', boxShadow: 'none', borderRadius: 26 } : {}} role="dialog" aria-label={`Group chat: ${groupName}`}>
      {/* Header */}
